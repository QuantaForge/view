...
class DynamicComponent extends Component
{
     ...
     *
     * @var \QuantaForge\View\Compilers\BladeTagCompiler
     */
     ...
     *
     * @return \QuantaForge\Contracts\View\View|string
     */
    public function render()
    {
        $template = <<<'EOF'
<?php extract(collect($attributes->getAttributes())->mapWithKeys(function ($value, $key) { return [QuantaForge\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]; })->all(), EXTR_SKIP); ?>
{{ props }}
...
EOF;
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__quantaforge_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
     ...
     *
     * @return \QuantaForge\View\Compilers\ComponentTagCompiler
     */
