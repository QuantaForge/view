...
namespace QuantaForge\View\Compilers;
...
use QuantaForge\View\AnonymousComponent;
use QuantaForge\View\DynamicComponent;
...
class ComponentTagCompiler
{
     ...
     * @param  \QuantaForge\View\Compilers\BladeCompiler|null  $blade
     * @return void
     */
    ...
    protected function componentString(string $component, array $attributes)
    {
        ...
        if (! class_exists($class)) {
            $view = Str::startsWith($component, 'mail::')
                ? "\$__env->getContainer()->make(QuantaForge\\View\\Factory::class)->make('{$component}')"
                : "'$class'";
...
<?php if (isset($attributes) && $attributes instanceof QuantaForge\View\ComponentAttributeBag && $constructor = (new ReflectionClass('.$class.'::class))->getConstructor()): ?>
...
<?php $component->withAttributes(['.$this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class).']); ?>';
     ...
     *
     * @param  \QuantaForge\Contracts\View\Factory  $viewFactory
     * @param  string  $component
     * @return string|null
     */
        ...
        return preg_replace_callback(
            '/@(class)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'class') {
                    ...
                    return ":class=\"\QuantaForge\Support\Arr::toCssClasses{$match[2]}\"";
                }
        ...
        return preg_replace_callback(
            '/@(style)(\( ( (?>[^()]+) | (?2) )* \))/x', function ($match) {
                if ($match[1] === 'style') {
                    ...
                    return ":style=\"\QuantaForge\Support\Arr::toCssStyles{$match[2]}\"";
                }
        ...
        return collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \QuantaForge\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
